...
class ModuloExemploSql
{
    ...
    public function getDadosSql($cod)
    {
        ...
        $qb->select('*')
           ->from('modulo_exe', 'a')
           ->where('moduloExeCod = :moduloExeCod')
           ->setParameter('moduloExeCod', $cod, \PDO::PARAM_INT);

        return $qb;

    }

    public function getDadosUploadSql($cod)
    {
        ...
        $qb->select('*')
           ->from('_upload', 'a')
           ->where('uploadCodReferencia = :uploadCodReferencia')
           ->andWhere('moduloCod = :moduloCod')
           ->setParameters(['moduloCod' => $moduloCod, 'uploadCodReferencia' => $cod]);
